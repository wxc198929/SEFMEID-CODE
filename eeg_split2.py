...
# 参数设置
# -----------------------------
excel_path = r"...\SEFMID\Raw data\EEG_1_19.xlsx" # 选择调整后事件打标文件
...
required_cols = {'subject', 'onset', 'duration', 'description'}
...
for subject, group in df.groupby('subject'):
    ...
    print(f"EEG 数据形状: {data.shape}, 通道数: {n_channels}, 时间点数: {n_times}")
    ...
    for i, row in group.iterrows():
        ...
        print(f"段 {len(segment_list)}: onset={onset}s, 描述={desc}")
    ...
    if len(segment_list) > 0:
        ...
        save_file = os.path.join(save_dir, f"{int(subject)}_segments_eeg.mat")
